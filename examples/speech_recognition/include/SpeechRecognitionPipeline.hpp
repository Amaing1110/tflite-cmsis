...
public:
     ...
     */
    ASRPipeline(tflite::MicroInterpreter    * executor,
    ...
    void Inference(const std::vector<T>& preprocessedData, common::InferenceResults<int8_t>& result) 
    {
        ...
        TfLiteTensor* output = m_executor->output(0);

        std::copy_n(preprocessedData.data(), data_bytes,
              tflite::GetTensorData<int8_t>(input));
        m_executor->Invoke();
        //std::copy_n(tflite::GetTensorData<int8_t>(output),  output->dims->data[output->dims->size-1], result.data());
    }
...
protected:
    tflite::MicroInterpreter    *  m_executor;
